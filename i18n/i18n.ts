...
const cache: Record<
	Locale,
	Record<string, string | { one: string; many: string }>
> = {
	en: {},
	es: {},
};
...
export async function init() {
	if (typeof window !== "undefined") {
		throw new Error(
			"i18n initialization within the browser is not implemented yet",
		);
	} else {
...
export function t(
	...
	const lang =
		(typeof _options === "string" ? _options : _locale) ?? locale.get();
